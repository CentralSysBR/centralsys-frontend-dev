            ...
            {!loadingCaixa && (
              <div className="hidden md:flex items-center gap-3 px-4 py-1.5 rounded-full border border-gray-100 bg-gray-50">
                ...
                <span className="text-[11px] font-bold text-gray-500 uppercase tracking-tight">
                  Caixa {CaixaAberto ? 'Operando' : 'Fechado'}
                </span>
              </div>
            )}
      ...
      <div className="flex max-w-7xl mx-auto">
        ...
        >
          ...
          <nav className="p-4 space-y-2">
            ...
            <div className="pt-10 space-y-2">
              <p className="px-4 text-[10px] font-bold text-gray-500 uppercase">
                Gestão de Caixa
              </p>
        ...
        <main className="flex-1 p-4 lg:p-8">
          <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Olá, <span className="font-semibold text-gray-700">{userName}</span>. O que deseja fazer hoje?
              </p>
            </div>

            {!CaixaAberto && !loadingCaixa && (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6">
            {menuItems.map((item) => {
              const isPdvBloqueado =
                item.path === '/pdv' && !CaixaAberto;

              return (
                <div 
                  key={item.title}
                  onClick={() =>
                    !isPdvBloqueado && navigate(item.path)
                  }
                  className={`
                    relative overflow-hidden bg-white p-5 lg:p-7 rounded-3xl border border-gray-100 shadow-sm transition-all
                ...
                >
                  <div
                    className={`${item.color} w-12 h-12 lg:w-14 lg:h-14 rounded-2xl flex items-center justify-center text-white mb-4 shadow-lg shadow-gray-100`}
                  >
                    {item.icon}
                  </div>
