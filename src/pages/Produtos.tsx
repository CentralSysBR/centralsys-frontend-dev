...
export default function Produtos() {
  ...
  async function confirmarEntrada() {
    ...
    try {
      ...
      await api.patch(`/produtos/${produtoSelecionado.id}/estoque`, {
        quantidadeAdicionada: qtdParaEnviar,
  ...
  const normalizarTexto = (txt: string) => txt.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
  const produtosFiltrados = produtos.filter(p => {
    const termo = normalizarTexto(busca);
    return normalizarTexto(p.nome).includes(termo) || (p.codigoBarras && p.codigoBarras.includes(busca));
  });

  const podeSalvarEntrada = produtoSelecionado && (qtdEntrada > 0 || novoPrecoVenda !== Number(produtoSelecionado.precoVenda));
